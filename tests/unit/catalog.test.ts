...
describe("verifyCatalogSnapshot", () => {
  test("verifies signed bytes and returns trusted state", () => {
    ...
    const result = verifyCatalogSnapshot(
      input.indexBytes,
...
describe("normalizeCatalog", () => {
  test("selects the latest non-yanked version and preserves history", () => {
    ...
    expect(plugin.installCommand).toBe(
      "sudo ohtools plugin install system-base",
    );
...
describe("mayUsePinnedBootstrap", () => {
  test("allows first deploy only for a missing published state", () => {
    expect(
      mayUsePinnedBootstrap(
        new Error("upstream request failed with HTTP 404"),
        true,
      ),
    ).toBe(true);
    expect(
      mayUsePinnedBootstrap(
        new Error("upstream request failed with HTTP 500"),
        true,
      ),
    ).toBe(false);
    expect(mayUsePinnedBootstrap(new Error("offline"), false)).toBe(true);
  });
});
