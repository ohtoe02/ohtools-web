            ...
            items: {
              ...
              properties: {
                ...
                manifest: {
                  type: "object",
                  additionalProperties: false,
                  required: ["protocol_version", "name", "version", "commands"],
                  properties: {
...
export function verifyCatalogSnapshot(
  ...
  if (signatureBytes.byteLength > maxSignatureBytes) {
    throw new Error(
      `catalog signature envelope exceeds ${maxSignatureBytes} bytes`,
    );
  }
  ...
  if (!validateEnvelope(envelope)) {
    throw new Error(
      `catalog signature schema rejected: ${ajv.errorsText(validateEnvelope.errors)}`,
    );
  }
  ...
  if (!validateCatalog(decoded)) {
    throw new Error(
      `catalog schema rejected: ${ajv.errorsText(validateCatalog.errors)}`,
    );
  }
...
function validateCatalogSemantics(index: CatalogIndexV1): void {
  ...
  for (const plugin of index.plugins) {
    ...
    if (plugin.homepage)
      validateCredentialFreeHttps(plugin.homepage, "homepage");

    const versions = new Set<string>();
    for (const version of plugin.versions) {
      if (versions.has(version.version)) {
        throw new Error(
          `plugin ${plugin.name} has duplicate version ${version.version}`,
        );
      }
      ...
      if ((version.manifest.description ?? "") !== (plugin.description ?? "")) {
        throw new Error(
          `plugin ${plugin.name} description does not match manifest`,
        );
      }
...
function validateDescription(description: string): void {
  if (
    ...
    throw new Error(
      "catalog description must be a single line of at most 512 bytes",
    );
  }
}
